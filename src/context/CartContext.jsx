...
export const CartProvider = ({ children }) => {
  ...
  const getCartTotal = cart.reduce((sum, item) => sum + item.quantity * item.price , 0)
  const clearCart = () => {
  setCart([]);
  };
  return (
    <CartContext.Provider value={{ cart, addToCart, removeFromCart, totalItems, clearCart, getCartTotal }}>
      {children}
